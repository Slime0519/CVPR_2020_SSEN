...
from Models.show.Dynamic_offset_estimator_concat_show import Dynamic_offset_estimator_concat_show
...
class Deformable_Conv_Block(nn.Module):
    def __init__(self,input_channels,mode):
        ...
        if mode == "concat":
            self.offset_estimator = Dynamic_offset_estimator_concat_show(input_channelsize=input_channels * 2)
        else:
    ...
    def forward(self,lr_features, hr_features, modelname , showmode = False, num_block =None):
        ...
        if showmode:
            ...
            saveoffset(estimated_offset,modelname = modelname, foldername="resultoffset_deformconv{}".format(num_block), istensor = True)
