...
class EDSR_baseline_X4(nn.Module):
    ...
    def forward(self,lrinput,refinput):
        ...
        ref_feature = self.downsample_conv1(refinput)
        ref_feature = self.relu(ref_feature)
        ref_feature = self.downsample_conv2(ref_feature)
        ref_feature = self.relu(ref_feature)
        ref_feature = self.feature_extraction_network(ref_feature)
    ...
    def load_pretrained_model(self):
        print(os.path.abspath('./edsr_baseline_x4-6b446fab.pt'))
        dict = torch.load('Modules/EDSR_pretrained_baseline/edsr_baseline_x4-6b446fab.pt')
        self.EDSR.load_state_dict(dict, strict=False)
